...
Groups where $AB$=$BA$ are called *abelian*. In abelian groups, two elements of finite order ($A$ and $B$) yield a third element of finite order ($AB$). As we have seen, non-abelian groups are a different story. Rotations in 3D are non-abelian (when we rotate along different axes).
